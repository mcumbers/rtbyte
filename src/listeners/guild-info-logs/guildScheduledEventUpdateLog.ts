...
export class UserEvent extends Listener {
	public async run(oldEvent: GuildScheduledEvent, event: GuildScheduledEvent) {
		...
		return this.container.client.emit(CustomEvents.GuildLogCreate, logChannel, this.generateGuildLog(oldEvent, event, auditLogEntry));
	}
	...
	private generateGuildLog(oldEvent: GuildScheduledEvent, event: GuildScheduledEvent, auditLogEntry: GuildAuditLogsEntry | null) {
		...
		if ((oldEvent.entityType !== event.entityType) || oldEvent.channel !== event.channel) {
			...
			embed.addBlankFields({ name: 'Location Changed', value: `${oldEventLocation} -> ${eventLocation}`, inline: false });
		}
		...
		if (oldEvent.scheduledStartTimestamp !== event.scheduledStartTimestamp) {
			...
			embed.addBlankFields({ name: 'Start Time Changed', value: `${oldStartTimestamp} -> ${startTimestamp}`, inline: false });
		}
		...
		if (oldEvent.scheduledEndTimestamp !== event.scheduledEndTimestamp) {
			...
			embed.addBlankFields({ name: 'End Time Changed', value: `${oldEndTimestamp} -> ${endTimestamp}`, inline: false });
		}

		if (oldEvent.description !== event.description) {
			embed.addBlankFields({ name: 'Description Changed', value: `\`\`\`diff\n-${oldEvent.description}\n+${event.description}\n\`\`\``, inline: false });
		}

		if (oldEvent.image !== event.image) {
			if (!oldEvent.image) embed.addBlankFields({ name: 'Image Added', value: `[New Image](${event.coverImageURL()})`, inline: true });
			if (!event.image) embed.addBlankFields({ name: 'Image Changed', value: 'Image Removed', inline: true });
			if (oldEvent.image && event.image) embed.addBlankFields({ name: 'Image Changed', value: `[New Image](${event.coverImageURL()})`, inline: false });
		}
