...
export class UserEvent extends Listener {
	...
	private async generateGuildLog(role: Role, auditLogEntry: GuildAuditLogsEntry | null) {
		...
		if (role.color) embed.addBlankFields({ name: 'Color', value: `${role.hexColor}`, inline: true });
		if (role.unicodeEmoji) embed.addBlankFields({ name: 'Emoji', value: role.unicodeEmoji as string, inline: true });
		if (role.hoist) embed.addBlankFields({ name: 'Role Display', value: 'Separated', inline: true });
		if (role.mentionable) embed.addBlankFields({ name: 'Role Status', value: 'Mentionable', inline: true });

		if (role.tags) {
			if (role.tags.botId) embed.addBlankFields({ name: 'Bot Role For', value: `<@${role.tags.botId}>`, inline: true });
			...
			if (role.tags.premiumSubscriberRole) embed.addBlankFields({ name: 'Role Type', value: 'Server Booster Role', inline: true });
		...
		if (permDifferences.added.length) {
			embed.addBlankFields({ name: 'Permissions', value: `\`\`\`diff\n${[...permDifferences.added.map((str) => `+ ${str}`)].join('\n')}\n\`\`\``, inline: false });
		}
